module Remoppy
  class Client
    ...
    def client
      @client ||= Remoppy::Remotty::Api.new.realtime
    end

    def start
      setup

      Debug.log 'Remoppy start'
    ...
    def hear(pattern, options = {}, &block)
      register_callback(:hear, :comment, Listener::TextListener.new(pattern, options, &block))
    end
    ...
    def respond(pattern, options = {}, &block)
      ...
      register_callback(:respond, :comment, Listener::TextListener.new(pattern, options, &block))
    end
