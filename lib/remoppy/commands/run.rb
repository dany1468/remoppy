module Remoppy
  module Commands
    class Run
      ...
      def load_dsl
        return if Remoppy.configuration.dsl == :disabled
        require 'remoppy/dsl'
      end

      def load_config
        file = File.expand_path Remoppy.configuration.config_file_path, Dir.pwd
        begin
          require file
        rescue LoadError
      ...
      def load_directory(dir_name, &block)
        dir_path = Remoppy.configuration.send dir_name.to_sym
